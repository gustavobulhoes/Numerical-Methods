%% Métodos Numéricos 2018 - Lista 1 - questão 5
...
ro = 0.99;           % parâmetro, coeficiente do AR(1)
...
xlabel('Tempo');
ylabel('Processo');
title('Comparativo das simulações do AR(1) contínuo e discretizado - N=9');
...
xlabel('Tempo');
ylabel('Processo');
title('Comparativo das 1000 primeiras simulações do AR(1) contínuo e discretizado - N=9');
legend('Contínuo', 'Tauchen', 'Rouwenhorst');
...
%% Questão 3 - Simulando para 10000 períodos e comparando - N = 20
...
%%%%%%% utilizar o mesmo e das questões 1-4
% e = ones(1,time);
% 
% for t = 1:time     % epslon (normal padrão)
%     e(t) = randn;
...
xlabel('Tempo');
ylabel('Processo');
title('Comparativo das simulações do AR(1) contínuo e discretizado - N=20');
legend('Contínuo', 'Tauchen', 'Rouwenhorst');
...
xlabel('Tempo');
ylabel('Processo');
title('Comparativo das 1000 primeiras simulações do AR(1) contínuo e discretizado - N=20');
legend('Contínuo', 'Tauchen', 'Rouwenhorst');
...
%% Questão 4 - Estimando AR(1) com base nos dados simulados - N = 20

% Temos que ro_hat = cov(yt, yt-1)/var(yt) e sigma2_hat = (1-ro^2)var(yt)

ro_hat_tauchen = cov(tauchen(1:time-1,1), tauchen(2:time,1))/var(tauchen(1:time,1));
ro_hat_tauch20 = ro_hat_tauchen(1,2)

ro_hat_rouwenhorst = cov(rouwenhorst(1:time-1,1), rouwenhorst(2:time,1))/var(rouwenhorst(1:time,1));
ro_hat_rouwen20 = ro_hat_rouwenhorst(1,2)

sigma2_hat_tauchen = (1 - ro_hat_tauchen(1,2)^2)*var(tauchen(1:time,1));
sigma_hat_tauch20 = sqrt(sigma2_hat_tauchen)

sigma2_hat_rouwenhorst = (1 - ro_hat_rouwenhorst(1,2)^2)*var(rouwenhorst(1:time,1));
sigma_hat_rouwen20 = sqrt(sigma2_hat_rouwenhorst)
